...
If we need deploy and up all this microservices to production server, will be create ci-cd piplenes:

#it all will work as one chain, but I will be wright about this processes   

git-jenkins-stage server- deploy full code and docker-containers
# on this server we can use for example, and check our web resource, services, fronend and backend processes after test request  


                                                                     
git - jenkins - test - deploy (if haven't problem and alerts - continue pipeline! If have issue- stop- notificate all team members!) : 

-  if docker images  was rebuild - redeploy to docker repo

- updated \ repaired \ release code can deploy to storage for advanced saving
                                                    
- updated \ repaired \ release code will deploy to production server, and  mounted to catalogs started docker microservices -services will reload 
                                                           


# On the production server will be use on configuration web services - reverse proxy , and ssl sertificates. this resources will listen
# on the port 443 or other needed for advanced micro-services
# In docker-compose.yml can up logic block, which can periodical check updates docker image on docker repo, which it can firstly pulled
# After changed state this image on the docker repo , docker-compose will be reloaded and docker images will be downloading and reloading
